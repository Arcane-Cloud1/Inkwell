    ...
    <AppLayout>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-lg text-paper">草稿</h3>
          {drafts.filter((d) => !d.remoteSha).length > 0 && (
            <span className="font-mono text-[11px] text-paper-dim">
              {drafts.filter((d) => !d.remoteSha).length} 篇草稿
            </span>
          )}
        </div>
              ...
              .map((draft) => (
                <li key={draft.id}>
                  ...
                  >
                    <div
                      className={cn(
                        "flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-amber-300/15 bg-ink-900/60 text-amber-300/70",
                      )}
                    >
